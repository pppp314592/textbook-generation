...
3. **A4印刷CSS**
   - **ヘッダ・フッタの抑制**: 印刷時にヘッダへ日時、フッタへURL・ファイルパス・ページ番号が出ないようにする。`@page` には margin box を定義しない（`@top-center` 等を置かない）。ブラウザ印刷ダイアログでも「ヘッダーとフッター」のチェックを外すよう案内する。
   - **PDF化のヘッダ・フッタ抑制（重要）**: 新しいChromium（Chrome 131以降・Edge等）では `--print-to-pdf-no-header` フラグが無視されることがある。`html-to-pdf.ps1` はフラグに依存せず、**DevTools Protocol（CDP）の `Page.printToPDF` を `displayHeaderFooter: false` で呼び出す**ため、ブラウザのバージョンに関わらずヘッダ（作成日時）・フッタ（ファイルパス・URL・ページ番号）は出力されない。この方法を他のツールで置き換えないこと。
   - 以下のCSSを必ず含める:
...
5. **PDF化**
   ...
   - スクリプトは自動でChromiumまたはEdgeを見つけ、ヘッドレスで起動してCDPの `Page.printToPDF`（`displayHeaderFooter: false`）でPDF化する。見つからない場合は `-Browser "C:\...\chrome.exe"` でパスを指定する。
   - 生成されたPDFにヘッダ・フッタ（作成日時・ファイルパス・URL・ページ番号）が出ていた場合は、**必ず `html-to-pdf.ps1` で再生成すること**。ヘッダ・フッタを消すために別のPDF化ツールやブラウザの「名前を付けて保存→PDF」に切り替えない（それらはヘッダ・フッタが付くため）。

6. **確認**
   - 生成されたPDFのファイルサイズと存在を確認し、ページ数（おおよそ）を報告する。
   - 表が横にはみ出していないか、ページ内で切れそうな表・コードがないかを確認し、必要ならCSSやレイアウトを調整して再生成する。
   - **ヘッダ・フッタに日時・URL・ファイルパス・ページ番号が出ていないことを確認する**（PDFの1ページ目〜数ページ目を確認。出ている場合は `html-to-pdf.ps1`（CDP `Page.printToPDF` + `displayHeaderFooter: false`）で再生成する。`@page` margin boxが無いかも確認）。
